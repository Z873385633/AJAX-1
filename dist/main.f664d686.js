...
getPage.onclick = function () {
  ...
  request.open("GET", "/page".concat(n + 1, ".json"));
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4) {
      if (request.status >= 200 && request.status < 300) {
        ...
        document.body.appendChild(script); //插入body里
      } else {
        alert('加载JS失败');
      }
    }
...
getCSS.onclick = function () {
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4) {
      if (request.status >= 200 && request.status < 300) {
        var style = document.createElement('style'); //创建script标签

        style.innerHTML = request.response; //填写script内容

        document.head.appendChild(style); //插入body里
      } else {
        alert('加载CSS失败');
      }
    }
...
//# sourceMappingURL=main.f664d686.js.map